...
class Main {
    ...
    static void updateLocations(List < List < String >> map, String userAnswer) {
        ...
        Integer mapIndex = 0;
        Integer rowIndex = 0;
        String correctedAnswer = userAnswer;
        Scanner scan = new Scanner(System.in); // Create a Scanner object
        for (int i = 0; i < 5; i++) {
            rowIndex = map.get(i).indexOf("|" + correctedAnswer + "       |");
            if (rowIndex != -1) {
                mapIndex = i;
                while (map.get(mapIndex).get(rowIndex + 6).equals("|    O    |")) {
                    //System.out.println(map.get(mapIndex).get(rowIndex+6));
                    //System.out.println("|" + correctedAnswer + "       |");
                    System.out.println("Invalid Entry, another character already located there.");
                    System.out.println("Enter location to move to:");
                    correctedAnswer = scan.nextLine();
